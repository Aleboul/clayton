...
samples of length n_samples (a list of int) where we compute the w-madogram on each. We are able
...
class MonteCarlo:
    ...
    ------
        ...
        n_samples (list of int or [int]) : multiple length of sample.
    ...
    def __init__(self, n_iter=None, n_samples=None, weight=None,
    ...
    def _wmado(self, mat, miss, corr=True):
        """
            ...
            Inputs
            ------
            mat (array([float]) of n_samples \times dim) : a matrix
    ...
    def _gen_missing(self):
        """This function returns an array max(n_samples) times dim of binary indicate missing values.
        ...
        We simulate it by generating a sample (U_0, dots, U_{dim-1}) of shape (n_samples, dim)
        ...
        Ouputs
        ------
        Array of shape (n_samples, dim) with the ith of the jth column equals 1
        if we observe mat_{ij}, 0 otherwise.
        """
        if self.copula_miss is None:
            return np.array([np.random.binomial(1,
                                                self.matp[j][j], np.max(self.n_samples))
                             for j in range(0, self.dim)]).reshape(np.max(self.n_samples),
                                                                   self.dim)
        sample_ = self.copula_miss.sample_unimargin()
        miss_ = np.array([1 * (sample_[:, j] <= self.matp[j][j])
                         for j in range(0, self.dim)]).reshape(np.max(self.n_samples), self.dim)
        return miss_
    ...
    def finite_sample(self, inv_cdf, corr=True):
        ...
        for miter in range(self.n_iter):
            wmado_store = np.zeros(len(self.n_samples))
            obs_all = self.copula.sample(inv_cdf)
            miss_all = self._gen_missing()
            for i in range(0, len(self.n_samples)):
                obs = obs_all[:self.n_samples[i]]
                miss = miss_all[:self.n_samples[i]]
                wmado = self._wmado(obs, miss, corr)
                wmado_store[i] = wmado

            output_cbind = np.c_[wmado_store, self.n_samples]
